...
// ── Conteúdo teórico exibido antes de cada missão ────────────────────────────

const THEORY = {
  1:  'O Git registra o autor de cada commit. Por isso, antes de versionar qualquer código, você precisa configurar seu nome e e-mail. Essas informações aparecem no histórico para toda a equipe.',
  2:  'O e-mail é vinculado ao seu nome no histórico de commits e é usado para integração com plataformas como o GitHub. Configure-o uma vez globalmente e todos os seus repositórios o usarão automaticamente.',
  3:  'O Git pode colorir sua saída: arquivos não rastreados em vermelho, staged em verde, etc. Com <code>color.ui auto</code>, ele detecta automaticamente se o terminal suporta cores e as ativa — muito mais legível!',
  4:  'Um repositório Git é uma pasta com superpoderes: ela rastreia cada mudança feita nos arquivos ao longo do tempo. O <code>git init</code> cria a pasta oculta <code>.git</code> — o coração do repositório.',
  5:  '<code>git status</code> é o comando mais usado no dia a dia. Ele responde à pergunta "o que está acontecendo agora?": quais arquivos foram modificados, o que está na staging area e o que ainda não foi rastreado.',
  6:  'O Git classifica arquivos em três estados: <strong>untracked</strong> (arquivo novo, nunca visto), <strong>modified</strong> (alterado desde o último commit) e <strong>staged</strong> (preparado para o próximo commit).',
  7:  'A staging area é uma zona intermediária entre o seu disco e o histórico de commits. Com ela, você escolhe exatamente quais mudanças vão para o próximo commit — é esse controle que <code>git add</code> dá a você.',
  8:  'Um commit é como tirar uma fotografia do projeto. Ele registra permanentemente o estado dos arquivos naquele momento, junto com autor, data e uma mensagem descritiva. Você pode voltar a qualquer commit no futuro.',
  9:  '<code>git diff</code> mostra exatamente o que mudou, linha por linha. Linhas com <code>+</code> foram adicionadas, com <code>-</code> foram removidas. É essencial para revisar suas mudanças antes de fazer o staging.',
  10: 'Existem dois momentos de revisão: antes do <code>git add</code> (use <code>git diff</code>) e depois do <code>git add</code> (use <code>git diff --staged</code>). O segundo mostra exatamente o que vai entrar no próximo commit.',
  11: 'Um histórico saudável tem muitos commits pequenos e bem descritos. Cada commit deve ter um único propósito expresso na mensagem — isso facilita encontrar bugs e entender o que aconteceu no projeto.',
  12: 'Quando há vários arquivos para adicionar, <code>git add .</code> adiciona tudo de uma vez. Use com cautela: verifique com <code>git status</code> antes para garantir que não vai incluir arquivos indesejados.',
  13: 'A staging area é reversível. Se você adicionou algo por engano, <code>git reset &lt;arquivo&gt;</code> remove-o da staging area sem apagar o arquivo em disco. Você mantém controle total sobre o que entra em cada commit.',
  14: 'Commits pequenos e focados são uma boa prática de engenharia. Um commit deve conter apenas mudanças relacionadas a uma única coisa — uma feature, um fix ou uma refatoração. Isso facilita code review e depuração.',
  15: '<code>git mv</code> renomeia ou move arquivos já versionados. Ao contrário do <code>mv</code> do sistema, o Git entende que é uma renomeação (não exclusão + criação) e preserva o histórico completo do arquivo.',
  16: 'Branches são linhas do tempo paralelas do projeto. Você pode criar um branch para desenvolver uma feature sem afetar o código estável do <code>main</code>. O asterisco (*) indica em qual branch você está atualmente.',
  17: 'Criar um branch é apenas criar um ponteiro para o commit atual. A partir daí, os commits neste branch ficam isolados. O branch original não é afetado até você fazer um merge.',
  18: 'Mudar de branch atualiza automaticamente os arquivos no disco para o estado daquele branch. É como "entrar" em outra linha do tempo — cada branch pode ter versões completamente diferentes dos arquivos.',
  19: 'Criar e imediatamente mudar para um novo branch é tão comum que existe um atalho: <code>git checkout -b nome</code>. Equivale a dois comandos em um, economizando tempo no fluxo de trabalho.',
  20: 'Commits feitos em um branch existem apenas nele — o <code>main</code> não os vê até que você faça um merge. Isso permite que várias pessoas trabalhem em features diferentes sem se atrapalhar.',
  21: 'Ao trocar de branch, o Git atualiza os arquivos no disco para refletir o estado daquele branch. Arquivos que existem em um branch podem simplesmente não existir em outro — esse é o isolamento dos branches.',
  22: 'Após fazer o merge, um branch cumpriu sua função e pode ser deletado. <code>git branch -d</code> exige que o branch já tenha sido mergeado. <code>-D</code> força a exclusão mesmo sem merge — use com cuidado.',
  23: 'O merge integra as mudanças de um branch em outro. Quando o branch de destino não teve commits novos desde a criação do feature branch, o Git faz um <em>fast-forward</em> — apenas avança o ponteiro, sem criar commit extra.',
  24: 'Quando ambos os branches têm commits independentes, o Git cria um "merge commit" — um commit especial com dois pais. Ele representa o ponto onde duas linhas do tempo convergiram no histórico.',
  25: 'Conflito ocorre quando dois branches modificam o mesmo trecho do mesmo arquivo de formas diferentes. O Git não tem como decidir qual versão está correta — ele para e pede para você resolver manualmente.',
  26: 'O Git marca conflitos com <code>&lt;&lt;&lt;&lt;&lt;&lt;&lt; HEAD</code>, <code>=======</code> e <code>&gt;&gt;&gt;&gt;&gt;&gt;&gt;</code>. Você edita o arquivo, escolhe (ou combina) o conteúdo final, depois faz <code>git add</code> e <code>git commit</code> para completar o merge.',
  27: '<code>git log</code> exibe o histórico completo: hash SHA-1, autor, data e mensagem de cada commit. O hash é o identificador único do commit — os primeiros 7 caracteres já são suficientes para referenciar qualquer um.',
  28: 'O <code>git log</code> tem dezenas de flags. A combinação <code>--oneline --graph --all</code> é a mais popular: compacta cada commit em uma linha, desenha a árvore de branches e inclui todos os branches do repositório.',
  29: '<code>git log &lt;arquivo&gt;</code> filtra o histórico para mostrar apenas os commits que modificaram aquele arquivo. A flag <code>--follow</code> faz o rastreamento continuar mesmo se o arquivo foi renomeado no passado.',
  30: '<code>git diff branch1...branch2</code> (com três pontos) mostra apenas as mudanças exclusivas de cada branch desde o ancestral comum. É ideal para revisar o que será integrado antes de fazer um merge.',
  31: '<code>git show &lt;hash&gt;</code> abre um commit específico e mostra tudo: metadados (autor, data, mensagem) e o diff completo de cada arquivo alterado. Você pode usar qualquer hash obtido do <code>git log</code>.',
  32: 'O <code>.gitignore</code> lista padrões de arquivos que o Git deve ignorar: logs, binários compilados, pastas de dependências como <code>node_modules</code>. Arquivos gerados automaticamente não devem ser versionados.',
  33: 'Um remote é um repositório hospedado em outro servidor (GitHub, GitLab). <code>origin</code> é o nome convencional para o remote principal. Com ele configurado, você pode enviar e receber código com sua equipe.',
  34: '<code>git push</code> envia seus commits locais para o repositório remoto. A flag <code>-u</code> configura o "upstream tracking" — após isso, <code>git push</code> sem argumentos sabe para onde enviar automaticamente.',
  35: '<code>git fetch</code> baixa as atualizações do remote <em>sem modificar</em> seus arquivos locais. É seguro — você pode inspecionar o que chegou antes de decidir integrar. Diferente do <code>git pull</code>, que integra imediatamente.',
  36: '<code>git pull</code> é a combinação de <code>git fetch</code> + <code>git merge</code>. Ele baixa as atualizações do remote e já as integra ao branch atual. É o comando mais usado para se manter sincronizado com a equipe.',
  37: 'O stash é uma pilha de rascunhos temporários. Quando você precisa trocar de contexto rapidamente — mudar de branch, aplicar um hotfix urgente — mas não quer commitar um trabalho incompleto, o <code>git stash</code> guarda suas mudanças para depois.',
  38: 'Você pode acumular múltiplos stashes, identificados por índice: <code>stash@{0}</code> (mais recente), <code>stash@{1}</code>, etc. <code>git stash list</code> mostra todos os rascunhos guardados na pilha.',
  39: '<code>git stash pop</code> restaura as mudanças do stash mais recente e o remove da pilha. Se quiser restaurar sem remover — útil para aplicar as mesmas mudanças em vários branches — use <code>git stash apply</code>.',
  40: '<code>git reset HEAD~1</code> desfaz o último commit mas mantém os arquivos no disco (modo <em>mixed</em>). É útil quando você commitou com uma mensagem ruim ou esqueceu de incluir algum arquivo — desfaz e refaz corretamente.',
};
